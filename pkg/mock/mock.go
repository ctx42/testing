...
// [Call.On], where specific arguments are expected, this allows the method to
...
//
//	Mock.On("Method", 1).Return(nil)
//	Mock.OnAny("MyOtherMethod").Return(errors.New("Some Error"))
...
// Called tells the mock object that a method has been called, and gets an
// array of arguments to return. Panics if the call is unexpected (i.e. not
// preceded by appropriate .On .Return() calls)
//
// If Call.until is set, blocks until the channel is closed or receives a
// message.
...
func (mck *Mock) Call(method string, args ...any) Arguments {
	// TODO(rz): Write tests simulation multiple goroutines calling.
...
// IsCallable returns nil if the method with given arguments can be called.
// Otherwise, returns an error explaining why the method is not callable.
func (mck *Mock) IsCallable(method string, args ...any) error {
	mck.mx.Lock()
	defer mck.mx.Unlock()
	mck.t.Helper()
...
//
// Note: this is used implicitly by Arguments.Diff if a Mock is passed. It
// exists because go's default %v formatting traverses the struct without
// acquiring the mutex, which is detected by go test -race.
func (mck *Mock) String() string {
	return fmt.Sprintf("%[1]T<%[1]p>", mck)
}
