...
func Test_OfType_match_tabular(t *testing.T) {
	tt := []struct {
		testN string
...
func Test_OfType_description_tabular(t *testing.T) {
	tt := []struct {
		testN string
...
func Test_MatchErr_tabular(t *testing.T) {
	...
	tt := []struct {
		...
		{"match", err0, err0, true},
		{"distinct errors", err0, err1, false},
		{"wrapped", err1, err3, true},
		{"wrapped multi 0", err0, err4, true},
		{"wrapped multi 1", err1, err4, true},
	}

	for _, tc := range tt {
		t.Run(tc.testN, func(t *testing.T) {
			// --- When ---
			have := MatchErr(tc.want).Match(tc.have)
...
func Test_MatchError_tabular(t *testing.T) {
	...
	tt := []struct {
		...
		{"1", "test error", err0, true},
		{"1", "other error", err0, false},
	}
