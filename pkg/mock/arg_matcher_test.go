...
func Test_NewArgMatcher(t *testing.T) {
	...
	t.Run("panics", func(t *testing.T) {
		...
		// --- Then ---
		assert.Panic(t, func() { NewArgMatcher(fn, "") })
	})
}

func Test_ArgMatcher_Desc(t *testing.T) {
	// --- Given ---
	want := "[mock.MyMatcher=func(int) bool]"

	// --- When ---
	am := NewArgMatcher(func(_ int) bool { return true }, want)

	// --- Then ---
	assert.Equal(t, want, am.Desc())
}

func Test_ArgMatcher_Match(t *testing.T) {
	t.Run("matcher true", func(t *testing.T) {
		// --- Given ---
		fn := func(have int) bool { return have == 42 }
		am := NewArgMatcher(fn, "mock.Custom")
	...
	t.Run("nil-able types", func(t *testing.T) {
		...
		// --- Then ---
		assert.True(t, NewArgMatcher(fnSlice, "mock.Custom").Match(nil))
		assert.True(t, NewArgMatcher(fnMap, "mock.Custom").Match(nil))
		assert.True(t, NewArgMatcher(fnPtr, "mock.Custom").Match(nil))
		assert.True(t, NewArgMatcher(fnItf, "mock.Custom").Match(nil))
		assert.True(t, NewArgMatcher(fnFn, "mock.Custom").Match(nil))
		assert.True(t, NewArgMatcher(fnCh, "mock.Custom").Match(nil))
	})
