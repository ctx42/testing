...
func (mck *Mocker) Generate(name string, opts ...Option) error {
	...
	buf.WriteString("\n\n")
	buf.WriteString(itf.genImports())
	buf.WriteString("\n\n")
	const format = "type %s struct {\n\t*mock.Mock\n\tt tester.T\n}"
	_, _ = fmt.Fprintf(buf, format, cfg.tgtName)
	buf.WriteString("\n\n")
	buf.WriteString(mck.genConstructor(cfg.tgtName))
	buf.WriteString("\n\n")
...
func (mck *Mocker) genConstructor(typeName string) string {
	const format = "func New%[1]s(t tester.T) *%[1]s {\n" +
		"\tt.Helper()\n" +
