...
// Strings computes the differences between two strings. The resulting edits
...
// Bytes computes the differences between two byte slices. The resulting edits
// respect rune boundaries.
...
func diffRunes(before, after []rune) []Edit {
	...
	// was passed in. Convert rune offsets to byte offsets.
	res := make([]Edit, len(diffs))
	lastEnd := 0
	utf8Len := 0
	for i, d := range diffs {
...
func runesLen(runes []rune) (l int) {
	for _, r := range runes {
		l += utf8.RuneLen(r)
	}
	return l
