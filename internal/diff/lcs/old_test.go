...
func TestRandOld(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		a := []rune(randstr(rnd, "abω", 16))
		b := []rune(randstr(rnd, "abωc", 16))
		seq := runesSeqs{a, b}
...
func genBench(set string, n int) []struct{ before, after string } {
	// Before and after for benchmarks. 24 strings of length n with before and
	// after differing at least once, and about 5%.
