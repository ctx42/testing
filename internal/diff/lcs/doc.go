...
D=1,k=1. In the same way, having labeled all the D vertices,
from a vertex labeled D,k find two vertices
tentatively labeled D+1,k-1 and D+1,k+1. There may be two on the same
diagonal, in which case take the one with the larger x.

Eventually the path gets to (N,M), and the diagonals on it are the LCS.

Here is the edit graph with the ends of D-paths labeled. (So, for instance,
0/2,2 indicates that x=2,y=2 is labeled with 0, as it should be, since the first
