...
// Edits are applied in order of start offset; edits with the same start offset
// are applied in the order they were provided.
//
// Apply returns an error if any edit is out of bounds or if any pair of edits
// is overlapping.
...
// ApplyBytes is like Apply, but it accepts a byte slice. The result is always
// a new array.
...
// validate checks that edits are consistent with src and returns the size of
// the patched output. It may return a different slice.
func validate(src string, edits []Edit) ([]Edit, int, error) {
	...
	// Check the validity of edits and compute the final size.
	size := len(src)
	lastEnd := 0
	for _, edit := range edits {
...
// SortEdits orders a slice of Edits by (start, end) offset. This ordering puts
// insertions (end = start) before deletions (end > start) at the same point
// but uses a stable sort to preserve the order of multiple insertions at the
// same point. (Apply detects multiple deletions at the same point as an error.)
...
// lineEdits expands and merges a sequence of edits so that each resulting edit
// replaces one or more complete lines. See ApplyEdits for preconditions.
func lineEdits(src string, edits []Edit) ([]Edit, error) {
	...
	// Do all deletions begin and end at the start of a line, and all
	// insertions end with a newline? (This is merely a fast path.)
	for _, edit := range edits {
...
expand:
	if len(edits) == 0 {
		return edits, nil // No edits (unreachable due to the fast path).
	}
...
func expandEdit(edit Edit, src string) Edit {
	...
	// Expand the end right to the end of the line.
	end := edit.End
	if end > 0 && src[end-1] != '\n' ||
		edit.New != "" && edit.New[len(edit.New)-1] != '\n' {
